...
def get_fish_xn_yn(source_x, source_y, radius, distortion):
    ...
    return source_x / (1 - (distortion*(radius**10))), source_y / (1 - (distortion*(radius**2)))
...
def parse_args(input_pic, output_pic, args=sys.argv[1:]):
    """Parse arguments."""
    ...
    parser.add_argument("-i", "--image", help="path to image file."
                        " If no input is given, the supplied example 'jinho.jpg' will be used.",
                        type=str, default=input_pic)

    parser.add_argument("-o", "--outpath", help="file path to write output to."
                        " format: <path>.<format(jpg,png,etc..)>",
                        type=str, default=output_pic)
...
def make_fisheye_photos():
    ...
    for dir_name in dir_list:
        ...
        for img in imgs:
            ...
            distored_img_path = dir_path + '\\f_' + img      
            args = parse_args(img_path, distored_img_path)

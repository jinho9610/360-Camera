...
def video_face_rec(load_data, input_video):
        ...
        if img_cropped_list is not None:
            ...
            for i, prob in enumerate(prob_list):
                if prob>0.90:
                    ...
                    if min_dist<0.90:
                        frame = cv2.putText(frame, name+' '+str(round(min_dist, 3)), (int(box[0]), int(box[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),1, cv2.LINE_AA)
                    
                    frame = cv2.rectangle(frame, (int(box[0]), int(box[1])) , (int(box[2]) ,int(box[3])), (255,0,0), 2)
            
            for face in faces:
                for i in range(len(face)):
                    frame = cv2.circle(frame, (int(face[i][0]), int(face[i][1])), 1, (0, 0, 255), -1)

    ...
    for img, idx in loader:
        ...
        if img_cropped_list is not None:
            ...
            if mode == 'train':
                ...
                #                 img, M, (img.shape[0], img.shape[1]))
                #             print(dest_dir + '/val/' +
                #                   cur_name + '/rotated_' + str(angle) + new_name + str(i) + '.jpg')
                #             cv2.imwrite(dest_dir + '/val/' +
                #                         cur_name + '/rotated_' + str(angle) + new_name + str(i) + '.jpg', rotated_img)
                #         else:
                #             M = cv2.getRotationMatrix2D(
                #                 (flipped_img.shape[0]/2, flipped_img.shape[1]/2), angle, 1)
                #             rotated_img = cv2.warpAffine(
                #                 flipped_img, M, (flipped_img.shape[0], flipped_img.shape[1]))
                #             print(dest_dir + '/val/' +
                #                   cur_name + '/rotated_f_' + str(angle) + new_name + str(i) + '.jpg')
                #             cv2.imwrite(dest_dir + '/val/' +
                #                         cur_name + '/rotated_f_' + str(angle) + new_name + str(i) + '.jpg', rotated_img)
